...
class CorsManager
{
    ...
    public function allowAnyOrigins(): CorsManager {
        $this->response->setHeader(IHttpHeaders::Access_Control_Allow_Origin, "*");
        return $this;
    }

    // all methods allowed
    public function allowAnyMethods(): CorsManager {
        $this->response->setHeader(IHttpHeaders::Access_Control_Allow_Methods, "*");
        return $this;
    }

    // all headers allowed
    public function allowAnyHeaders(): CorsManager {
        $this->response->setHeader(IHttpHeaders::Access_Control_Allow_Headers, "*");
        return $this;
    }
    ...
    public function allowOrigins(string ...$origins): CorsManager {
        if(($origin = $this->request->getHeader(IHttpHeaders::Origin)) && in_array($origin, $origins)) {
            $this->response->setHeader(IHttpHeaders::Access_Control_Allow_Origin, $origin);
        }
        return $this;
    }
    ...
    public function allowHeaders(string ...$headers): CorsManager {
        if($this->request->getHeader(IHttpHeaders::Access_Control_Request_Headers))
            $this->response->setHeader(IHttpHeaders::Access_Control_Allow_Headers, implode(", ", $headers) );
        return $this;
    }
    ...
    public function allowMethods(string ...$methods): CorsManager {
        if ($this->request->getHeader(IHttpHeaders::Access_Control_Request_Method))
            $this->response->setHeader(IHttpHeaders::Access_Control_Allow_Methods, implode(", ", $methods) );
        return $this;
    }
    ...
    public function supportsCredentials(bool $v) {
        ...
        $this->response->setHeader(IHttpHeaders::Access_Control_Allow_Credentials, ($v && $origin) ? "true" : "false");
    }

    public function maxAge(int $time) {
        if($this->request->getHeader(IHttpHeaders::Origin))
            $this->response->setHeader(IHttpHeaders::Access_Control_Max_Age, $time);
    }
